...
Benchmark 覆盖 100、1,000、10,000 词，短/长文本，无命中、首部/尾部命中及大量重叠，分别测量构建、`contains()`、`scan()` 和 `mask()`。可追加 `--iterations=100` 调整迭代数；运行结果以 JSON 输出。`contains()` 仍需完成输入校验；无白名单时可流式处理，在首个有效命中后提前结束匹配，有白名单时则需全文归一化和白名单定位。
